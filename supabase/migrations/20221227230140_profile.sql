...
create policy "Can view own user data." on profile for select using (next_auth.uid() = id);
create policy "Can update own user data." on profile for update using (next_auth.uid() = id);
...
create function public.handle_new_user()
    returns trigger as $$
begin
insert into public.profile (id, name, email, image)
